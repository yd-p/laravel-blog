                        ...
                        Forms\Components\Section::make('发布设置')
                            ->schema([
                                Forms\Components\Select::make('status')
                                    ->label('状态')
                                    ->options(\App\Enums\PostStatus::toSelectArray())
                                    ->required()
                                    ->default(\App\Enums\PostStatus::DRAFT->value)
                                    ->native(false),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('status')
                    ->label('状态')
                    ->badge()
                    ->formatStateUsing(fn (\App\Enums\PostStatus $state): string => $state->label())
                    ->color(fn (\App\Enums\PostStatus $state): string => $state->color())
                    ->icon(fn (\App\Enums\PostStatus $state): string => $state->icon())
                    ->sortable(),
